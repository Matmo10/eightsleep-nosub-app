...
export const TemperatureProfileForm: React.FC = () => {
  ...
  useEffect(() => {
    if (getUserTemperatureProfileQuery.isSuccess) {
      ...
      setValue("preheatEnabled", profile.preheatEnabled);
  ...
  const onSubmit = (data: TemperatureProfileForm) => {
    if (sleepDurationError && cycleEnabled) {
    ...
    const mutationData = {
      ...
      preheatEnabled: data.preheatEnabled,
  ...
  const onDelete = () => {
    if (
      window.confirm("Are you sure you want to delete your temperature profile?")
    ) {
  ...
  const SliderInput: React.FC<{
    ...
    info?: string;
    disabled?: boolean;
  }> = ({ name, label, control, info, disabled }) => (
    ...
    <div className="mx-auto mt-8 max-w-md rounded-lg bg-white p-6 shadow-xl">
      ...
      >
        ...
        <CheckboxInput
          ...
          control={control}
        />
        <div className={`pl-4 ${preheatEnabled ? "" : "hidden"}`}>
          <div>
            <label
              htmlFor="preheatTime"
              className="block text-sm font-medium text-gray-700"
            ...
            {errors.preheatTime && (
              <p className="mt-1 text-sm text-red-600">
                {errors.preheatTime.message}
              </p>
            )}
          </div>
          <SliderInput
            name="preheatLevel"
            label="Pre-heat Level"
            control={control}
            disabled={!preheatEnabled}
          />
        </div>
        ...
        <div className="flex justify-between">
          <Button
            ...
            disabled={
              updateProfileMutation.isPending || (!!sleepDurationError && cycleEnabled)
            }
          >
